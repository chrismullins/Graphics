...
static Eigen::Vector3f* vertexNormals = NULL;
static Eigen::Vector3f* vertexHalfVectors = NULL;
static Eigen::Vector3f* vertexLightVectors = NULL;
float* pixels = NULL;
...
void calculateVertexAttributes();
Eigen::Vector3f gouraudShade(float alphaV, float betaV, float gammaV, int triangleIndex);
Eigen::Vector3f perVertexShading(float alpha, float beta, float gamma, int triangleIndex);
...
void calculateVertexAttributes()
  {
  vertexNormals = (Eigen::Vector3f *) malloc(gNumVertices * sizeof(Eigen::Vector3f));
  vertexHalfVectors = (Eigen::Vector3f *) malloc(gNumVertices * sizeof(Eigen::Vector3f));
  vertexLightVectors = (Eigen::Vector3f *) malloc(gNumVertices * sizeof(Eigen::Vector3f));
  ...
  for (int i = 0; i < gNumVertices; ++i)
    ...
    Eigen::Vector3f lVec = lightPosition[0] - vertex;
    lVec.normalize();
    vertexLightVectors[i] = lVec;

    Eigen::Vector3f v = -vertex;
    v.normalize();
    Eigen::Vector3f vlSum = v + lVec;
    Eigen::Vector3f h = vlSum.normalized();
    vertexHalfVectors[i] = h;
    }
  }
...
Eigen::Vector3f perVertexShading(float alpha, float beta, float gamma, int triangleIndex)
  ...
  Eigen::Vector3f normalA = vertexNormals[k0];
  Eigen::Vector3f normalB = vertexNormals[k1];
  Eigen::Vector3f normalC = vertexNormals[k2];
  Eigen::Vector3f interpNormal = alpha*normalA + beta*normalB + gamma*normalC;

  Eigen::Vector3f halfA = vertexHalfVectors[k0];
  Eigen::Vector3f halfB = vertexHalfVectors[k1];
  Eigen::Vector3f halfC = vertexHalfVectors[k2];
  Eigen::Vector3f interpHalf = alpha*halfA + beta*halfB + gamma*halfC;

  Eigen::Vector3f lightA = vertexLightVectors[k0];
  Eigen::Vector3f lightB = vertexLightVectors[k1];
  Eigen::Vector3f lightC = vertexLightVectors[k2];
  Eigen::Vector3f interpLight = alpha*lightA + beta*lightB + gamma*lightC;
  ...
  interpLight.normalize();
  interpHalf.normalize();

  Eigen::Vector3f Ld;
  Ld << sphereKd[0](0)*fmaxf(0, interpNormal.dot(interpLight)),
        sphereKd[0](1)*fmaxf(0, interpNormal.dot(interpLight)),
        sphereKd[0](2)*fmaxf(0, interpNormal.dot(interpLight));

  Eigen::Vector3f La;
  La = sphereKa[0]*Ia;

  float npDot = pow(fmaxf(0, interpNormal.dot(interpHalf)),p);
  ...
  for (int y = ymin; y <= ymax; ++y)
    ...
    for (int x = xmin; x <=xmax; ++x)
      ...
      if (b >= 0 && g >= 0 && b+g <= 1)
        ...
        if (depthBuffer[512*y + x] < depth)
          {
          //Eigen::Vector3f attribute = gouraudShade(1-b-g,b,g,i);
          //colorPixel(x, y, attribute(0), attribute(1), attribute(2));
          Eigen::Vector3f intensity = perVertexShading(1-b-g,b,g,i);
          colorPixel(x,y, intensity(0), intensity(1), intensity(2));
...
int main(int argc, char** argv)
  ...
  glutCreateWindow("Per-Pixel Shading"); 
  create_scene();
